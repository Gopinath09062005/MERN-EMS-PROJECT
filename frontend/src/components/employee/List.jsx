    ...
    if (confirm) {
        try {
            ...
            if (response.data.success) {
                fetchEmployees(); // டேபிளை ரீஃப்ரெஷ் செய்ய
            }
        } catch (error) {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const data = response.data.employees
          .filter((emp) => emp.userId.role !== "admin") // Admin-ஐ மறைக்க
          .map((emp) => ({
            _id: emp._id,
            sno: sno++,
  ...
  return (
   <div className="p-4 md:p-6">
      ...
      <div className='mt-6 bg-white shadow-lg rounded-lg border border-gray-200'>
        {empLoading ? (
            <div className="text-center py-10 font-bold text-gray-500">Loading Employees...</div>
        ) : (
